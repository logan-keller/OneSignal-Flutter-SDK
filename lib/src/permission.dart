...
class OSPermissionState extends JSONStringRepresentable {
  ...
  String jsonRepresentation() {
    return convertToJsonString(
        {'hasPrompted': this.hasPrompted, 'provisional': this.provisional, 'status': this.status?.index ?? null});
  }
}
...
class OSPermissionStateChanges extends JSONStringRepresentable {
  ...
  OSPermissionStateChanges(Map<String, dynamic> json) {
    if (json.containsKey('from')) this.from = OSPermissionState(json['from'].cast<String, dynamic>());
    if (json.containsKey('to')) this.to = OSPermissionState(json['to'].cast<String, dynamic>());
  }

  String jsonRepresentation() {
    return convertToJsonString({'from': this.from.jsonRepresentation(), 'to': this.to.jsonRepresentation()});
  }
}
